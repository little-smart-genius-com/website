"""
REGEN ALL IMAGES V6 — Clean Slate Master Prompt + OG Images + ~60 Word Variations
1. Deletes ALL existing images from images/, images/thumbs/, AND images/og/
2. For each article, generates cover + img1..img4
...
DeepSeek Art Director is instructed to write ~40-60 word unique subject variations.
"""
...
from master_prompt import MASTER_PROMPT, build_prompt
...
ART_DIRECTOR_SYSTEM_V6 = (
    ...
    "This text will replace the [SUBJECT] placeholder in a master template. "
...
def ask_art_director(title, section_context, role):
    """Call DeepSeek for a ~60 word [SUBJECT] description."""
    user_msg = ART_DIRECTOR_USER_V6.format(
        ...
        try:
            resp = requests.post(
                ...
                json={
                    "model": "deepseek-chat",
                    "messages": [
                        {"role": "system", "content": ART_DIRECTOR_SYSTEM_V6},
...
def update_html_images(html_path, slug, new_files):
    ...
    inline_pattern = re.compile(r"src='\.\.\/images\/[^']*?\.(?:webp|jpg|png)'")
...
async def process_article(session, html_path, article_num, total, all_covers):
    ...
    if ok:
        ...
        og_img = img.copy() # original is up to 1200x675
        # crop to 1200x630 (center crop)
        width, height = og_img.size
        # just crop the top/bottom slightly
        left = 0
...
async def main():
    ...
    print("\n" + "=" * 65)
    print("  REGEN ALL IMAGES V6 — MASTER PROMPT + OG + UNIQUE VARIATIONS")
    print(f"  {len(html_files)} articles. Generating Cover, Thumbs, OG, and Img1-4.")
    print("=" * 65)
