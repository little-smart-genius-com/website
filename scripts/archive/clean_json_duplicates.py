...
# ⚠️  SAFETY GUARD v2 — V6 Compatibility Check
# ─────────────────────────────────────────────────────────────────────────────
# This script was created when V4 generated duplicate FAQ/CTA blocks.
# V6 (auto_blog_v6_ultimate.py) generates clean, non-duplicate content.
# Running this script on V6 articles will DELETE their FAQ sections and CTA boxes.
#
# This guard detects V6 articles and ABORTS to prevent data destruction.
# If you are certain you need to clean genuinely duplicate content in old V4 articles,
# comment out the guard below.
# ─────────────────────────────────────────────────────────────────────────────
def _v6_safety_check():
    script_dir = os.path.dirname(os.path.abspath(__file__))
    project_root = os.path.dirname(script_dir)
    posts_dir = os.path.join(project_root, 'posts')
    for f in glob.glob(os.path.join(posts_dir, '*.json'))[:5]:  # sample first 5
        try:
            with open(f, 'r', encoding='utf-8') as fh:
                data = json.load(fh)
            if data.get('version') == 'V6-Ultimate' or 'V6-Ultimate' in str(data.get('content', ''))[:200]:
                print("[ABORT] V6 articles detected in posts/.")
                print("        clean_json_duplicates.py is INCOMPATIBLE with V6 content.")
                print("        It would DELETE the FAQ and CTA sections generated by V6.")
                print("        If you need to clean old V4-era duplicates, manually disable this guard.")
                sys.exit(1)
        except Exception:
            pass
